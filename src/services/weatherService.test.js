...
import { getWeatherByCoordinates, getCitiesByName, getHourlyForecast, DEFAULT_CITIES, } from './weatherService';
describe('Weather Service', () => {
    ...
    describe('getWeatherByCoordinates', () => {
        ...
        it('should return weather data when the API call is successful', async () => {
            getWeatherByCoordinates.mockResolvedValueOnce(mockWeatherData);
        ...
        it('should throw an error when the API call fails', async () => {
            getWeatherByCoordinates.mockRejectedValueOnce(new Error('Failed to fetch weather data'));
            await expect(getWeatherByCoordinates(51.5074, -0.1278)).rejects.toThrow('Failed to fetch weather data');
        });
    });
    describe('getCitiesByName', () => {
        ...
        it('should return cities data when the API call is successful', async () => {
            getCitiesByName.mockResolvedValueOnce(mockCitiesData);
        ...
        it('should throw an error when the API call fails', async () => {
            getCitiesByName.mockRejectedValueOnce(new Error('Failed to fetch cities'));
    ...
    describe('getHourlyForecast', () => {
        ...
        it('should return hourly forecast when the API call is successful', async () => {
            getHourlyForecast.mockResolvedValueOnce(mockForecastData);
        ...
        it('should throw an error when the API call fails', async () => {
            getHourlyForecast.mockRejectedValueOnce(new Error('Failed to fetch hourly forecast data'));
            await expect(getHourlyForecast(51.5074, -0.1278)).rejects.toThrow('Failed to fetch hourly forecast data');
        });
    });
