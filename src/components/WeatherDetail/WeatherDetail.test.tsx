...
jest.mock('../TemperatureGraph/TemperatureGraph', () => ({
  TemperatureGraph: () => <div data-testid="temperature-graph">Temperature Graph</div>,
}));
...
const renderWithState = (
  ...
  render(
    <MemoryRouter initialEntries={['/city/123']} {...routerConfig}>
      <Routes>
        <Route path="/city/:cityId" element={<WeatherDetail />} />
      </Routes>
    </MemoryRouter>
  );
...
describe('WeatherDetail Component', () => {
  ...
  test('dispatches fetch weather action when city exists and status is idle', () => {
    ...
    const fetchWeatherForCityMock = jest.requireMock(
      '../../store/slices/weatherSlice'
    ).fetchWeatherForCity;
