...
jest.mock('../CityCard/CityCard', () => ({
  CityCard: jest.fn(({ city, onClick }) => (
    <div
      data-testid={`city-card-${city.id}`}
      className="city-card-mock"
...
describe('CityList Component', () => {
  ...
  test('renders empty list when no cities are provided', () => {
    const { container } = render(<CityList cities={[]} />);
  ...
  test('passes the onCityClick prop to each CityCard', () => {
    ...
    render(<CityList cities={mockCities} onCityClick={handleCityClick} />);
