...
jest.mock('../../components/CitySearch/CitySearch', () => ({
  CitySearch: () => <div data-testid="city-search">City Search Component</div>,
}));

jest.mock('../../components/CityList/CityList', () => ({
  CityList: ({ cities, onCityClick }: { cities: City[]; onCityClick: (city: City) => void }) => (
    <div data-testid="city-list">
      {cities.map((city) => (
        <button key={city.id} onClick={() => onCityClick(city)} data-testid={`city-${city.id}`}>
          {city.name}
        </button>
      ))}
    </div>
...
describe('HomePage Component', () => {
  ...
  const renderHomePage = (reduxState: Partial<RootState> = {}) => {
    const useSelector = jest.requireMock('react-redux').useSelector;
    useSelector.mockImplementation((selector: (state: RootState) => any) =>
      selector(reduxState as RootState)
    );
  ...
  test('displays empty state message when no cities', () => {
    ...
    expect(
      screen.getByText('No cities added yet. Search for a city above to add it to your list.')
    ).toBeInTheDocument();
  });
